...
function _renderWhyServiceList({
  ...
  switch (id) {
    ...
    default:
      return (
        <div className="flex flex-col justify-between max-h-[250px] contain-paint w-full h-full">
          <div className="flex justify-between items-center px-5 py-4">
            <h4 className="text-base md:text-xl lg:text-lg xl:text-xl font-medium whitespace-pre">
              {title}
            </h4>
            <span className="text-xl md:text-4xl lg:text-3xl xl:text-4xl font-medium">
              {number}
            </span>
          </div>
          <Image
            className={
              'w-full h-[160px] md:h-[200px] lg:h-[160px] object-cover'
            }
            src={src}
            alt={title}
            width={1280}
