...
const ContactUsModal: React.FC<ContactUsModalProps> = ({ click, handleClick }) => {
  ...
  return (
      <div className="absolute gap-5 px-6 py-8 rounded-3xl bg-contactUsModal bg-center bg-cover text-white flex flex-col z-[999] w-[90%] h-[660px] top-[10px] left-[5%]">
      <IoMdClose
        size={35}
        onClick={handleClick}
        className="absolute w-7 top-4 right-5 z-20 fill-white md:w-14 md:right-16 md:top-9 cursor-pointer"
      />
            ...
            <div className="flex flex-col gap-2">
              ...
              <Menu as="div" className="relative inline-block text-left">
                ...
                <MenuItems className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                  <div className="py-1">
                    {['Landing Page', 'E-Commerce', 'UI/UX Design', 'Business Website', 'CRM System', '3D Design'].map(option => (
                      <MenuItem key={option}>
                        {({ active }) => (
                          <p
                            onClick={() => {
